...
class AdminVerFacturas extends React.Component {
  data = [];
  descripcion = 1;
  ...
  handleVerDetalle = (num_fact) => {
    window.location.href = `/adminVerDetalleFactura/${num_fact}`;
  };
  render() {
    return (
              ...
              {this.data.map((factura) => {
                ...
                return (
                  <tr>
                    ...
                    <td>{factura.total}</td>
                    <td>
                      <Link to={`/adminVerDetalleFactura/${factura.num_fact}`}>
                        {" "}
                        Ver detalle
                      </Link>
                    </td>
...
export default AdminVerFacturas;
